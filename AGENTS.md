...
- **State Machine Protection:** Maintain the existing 5-phase teaching lifecycle (`intro` -> `concept` -> `example` -> `doubt` -> `transition`) in `src/backend/websocket/cherrySocket.ts` and `useLiveSession.ts`.
- **Modular Backend Architecture:**
  - `server.ts`: Lightweight HTTP & WebSocket orchestrator with Vite dev middleware.
  - `src/backend/config/gemini.ts`: Centralized Gemini AI client with telemetry, exponential backoff, and model fallback hierarchy.
  - `src/backend/state/sessionStore.ts`: Multi-session memory store, syllabus topic segmentation, and document state.
  - `src/backend/services/speechSynthesisService.ts`: 24kHz PCM-to-WAV conversion and multi-speaker dialogue synthesis.
  - `src/backend/routes/`: Dedicated REST route modules (`authRoutes`, `documentRoutes`, `studyRoutes`, `guideRoutes`, `pyqRoutes`, `podcastRoutes`).
  - `src/backend/websocket/`: Dedicated WebSocket handlers for all 4 live voice assistants (`cherrySocket`, `conciergeSocket`, `kiaraSocket`, `guideSocket`) and HTTP upgrade routing (`wsRoutes.ts`).

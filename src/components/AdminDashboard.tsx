...
export interface StudentCRMRecord {
  ...
  isPro?: boolean;
  subscriptionStatus?: "active" | "pending_verification" | "free" | "expired" | "suspended" | "pending" | "revoked" | string;
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const handleToggleReferralStatus = (studentId: string, status: "active" | "paused" | "suspended" | "frozen" | string) => {
    toggleStudentReferralStatus(studentId, status);
    setReferralUpdateCounter((c) => c + 1);
    if (inspectingReferralStudent && inspectingReferralStudent.studentId === studentId) {
      setInspectingReferralStudent((prev) => (prev ? { ...prev, status: status as any } : null));
    }
    onToast?.(
      status === "active"

...
interface AdminPayoutApprovalModalProps {
  ...
  mode: "approve" | "reject";
  onApprove: (arg1: string, arg2?: string) => void;
  onReject: (arg1: string, arg2?: string) => void;
  onToast?: (message: string, type?: "success" | "info" | "error" | "warning") => void;
}
...
export const AdminPayoutApprovalModal: React.FC<AdminPayoutApprovalModalProps> = ({
  ...
  const effectiveRecord = record || withdrawal;
  ...
  }, [isOpen, effectiveRecord, mode]);

  if (!isOpen || !effectiveRecord) return null;

  const handleCopyUpi = () => {
    navigator.clipboard.writeText(effectiveRecord.upiId);
    setCopiedUpi(true);
    onToast?.(`Copied UPI ID: ${effectiveRecord.upiId} 📋`, "info");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (mode === "approve") {
      ...
      onApprove(effectiveRecord.id, utrNumber.trim());
    } else {
      const finalReason = customReason.trim() ? customReason.trim() : selectedReason;
      onReject(effectiveRecord.id, finalReason);
    }
    onClose();
  };

  const upiPayLink = `upi://pay?pa=${encodeURIComponent(effectiveRecord.upiId)}&pn=${encodeURIComponent(
    effectiveRecord.studentName || "Scholar"
  )}&am=${effectiveRecord.amount}&cu=INR&tn=CherryAI_Scholarship_Payout`;
